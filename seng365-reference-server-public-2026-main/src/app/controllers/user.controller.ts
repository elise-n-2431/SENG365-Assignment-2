...
import {AuthenticatedIdRequestHandler} from "../types/expressTypes";
...
const logout: AuthenticatedIdRequestHandler = async (req, res): Promise<void> => {
    try {
...
const view: AuthenticatedIdRequestHandler = async (req, res): Promise<void> => {
    try {
...
const update: AuthenticatedIdRequestHandler = async (req, res): Promise<void> => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
            res.statusMessage = "Id must be an integer"
        ...
        if(res.locals.authId !== parseInt(req.params.id, 10)){
            res.status(403).send();
            return;
        }
