...
const getImage: IdRequestHandler = async (req, res): Promise<void> => {
    try {
...
const setImage: AuthenticatedIdRequestHandler = async (req, res): Promise<void> => {
    try{
        let isNew = true;
        const userId = parseInt(req.params.id, 10);
        if (isNaN(userId)) {
...
const deleteImage: AuthenticatedIdRequestHandler = async (req, res): Promise<void> => {
    try {
        const userId = parseInt(req.params.id, 10);
        if (isNaN(userId)) {
            res.statusMessage = "Id must be an integer"
